...
class Renderer {
    ...
    var projectionMatrix4D: matrix_float5x5 = matrix_float5x5(data: Array<Float>(repeating: 0.0, count: 25))
    var rotationX: Float = 0
    var rotationY: Float = 0
    var horizontalPlane = Plane4D.XW
    var verticalPlane = Plane4D.YW
    ...
    func loadMetal() {
        ...
        let path = Bundle.main.path(forResource: "Shader", ofType: "metal")
        let source = try! String(contentsOfFile: path!, encoding: .utf8)
        let defaultLibrary = try! device.makeLibrary(source: source, options: nil)
        ...
        imagePlaneVertexDescriptor.attributes[0].bufferIndex = Int(BufferIndices.MeshPositions.rawValue)
        
        // Texture coordinates.
        imagePlaneVertexDescriptor.attributes[1].format = .float2
        imagePlaneVertexDescriptor.attributes[1].offset = 8
        imagePlaneVertexDescriptor.attributes[1].bufferIndex = Int(BufferIndices.MeshPositions.rawValue)
        ...
        geometryVertexDescriptor.attributes[VertexAttributes.Position.rawValue].format = MTLVertexFormat.float4
        geometryVertexDescriptor.attributes[VertexAttributes.Position.rawValue].offset = 0
        geometryVertexDescriptor.attributes[VertexAttributes.Position.rawValue].bufferIndex = 0
        
        geometryVertexDescriptor.attributes[VertexAttributes.Color.rawValue].format = MTLVertexFormat.float4
        geometryVertexDescriptor.attributes[VertexAttributes.Color.rawValue].offset = 16
        geometryVertexDescriptor.attributes[VertexAttributes.Color.rawValue].bufferIndex = 0
        
        geometryVertexDescriptor.layouts[BufferIndices.MeshPositions.rawValue].stride = 32
        geometryVertexDescriptor.layouts[BufferIndices.MeshPositions.rawValue].stepRate = 1
        geometryVertexDescriptor.layouts[BufferIndices.MeshPositions.rawValue].stepFunction = MTLVertexStepFunction.perVertex
        ...
        do {
            ...
            NSLog("Failed to created anchor geometry pipeline state, error \(error)")
        }
    ...
    func loadAssets() {
        ...
        do {
            try cubeMesh = Hypercube(wireframe: true, device: device)
        } catch let error {
            NSLog("Error creating MetalKit mesh, error \(error)")
        }
    }
    ...
    func updateSharedUniforms(frame: ARFrame) {
        ...
        uniforms.pointee.projectionMatrix4D = projectionMatrix4D.cArray()
        
        //No lightning, because ARKit lightning doesn't work in 4D
    }
    ...
    func updateAnchors(frame: ARFrame) {
        ...
        for index in 0..<anchorInstanceCount {
            ...
            let modelMatrix4D = matrix_float5x5.rotation(radians: rotationX, plane: horizontalPlane) * matrix_float5x5.rotation(radians: rotationY, plane: verticalPlane)
            
            let viewMatrix4D = matrix_float5x5.translation(0.0, 0.0, 0.0, -16.0)
            let modelViewMatrix4D = (viewMatrix4D * modelMatrix4D).cArray()
            
            let anchorUniforms = anchorUniformBufferAddress.assumingMemoryBound(to: InstanceUniforms.self).advanced(by: index)
            anchorUniforms.pointee.modelMatrix = modelMatrix
            anchorUniforms.pointee.modelViewMatrix4D = modelViewMatrix4D
        }
    }
    ...
    func updateImagePlane(frame: ARFrame) {
        ...
        projectionMatrix4D = matrix_float5x5.perspective_4D(fovyRadians: radians_from_degrees(65))
    }
    ...
    func drawCapturedImage(renderEncoder: MTLRenderCommandEncoder) {
        ...
        renderEncoder.setVertexBuffer(imagePlaneVertexBuffer, offset: 0, index: Int(BufferIndices.MeshPositions.rawValue))
        
        // Set any textures read/sampled from our render pipeline
        renderEncoder.setFragmentTexture(CVMetalTextureGetTexture(textureY), index: Int(TextureIndices.IndexY.rawValue))
        renderEncoder.setFragmentTexture(CVMetalTextureGetTexture(textureCbCr), index: Int(TextureIndices.CbCr.rawValue))
    ...
    func drawAnchorGeometry(renderEncoder: MTLRenderCommandEncoder) {
        ...
        renderEncoder.setVertexBuffer(anchorUniformBuffer, offset: anchorUniformBufferOffset, index: Int(BufferIndices.InstanceUniforms.rawValue))
        renderEncoder.setVertexBuffer(sharedUniformBuffer, offset: sharedUniformBufferOffset, index: Int(BufferIndices.SharedUniforms.rawValue))
        renderEncoder.setFragmentBuffer(sharedUniformBuffer, offset: sharedUniformBufferOffset, index: Int(BufferIndices.SharedUniforms.rawValue))
        
        // Set mesh's vertex buffers
        renderEncoder.setVertexBuffer(cubeMesh.vertexBuffer, offset: 0, index: 0)
        
        // Draw each submesh of our mesh
        renderEncoder.drawPrimitives(type: cubeMesh.primitiveType, vertexStart: 0, vertexCount: cubeMesh.vertexCount, instanceCount: anchorInstanceCount)
